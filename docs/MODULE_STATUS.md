...
| **No public store-config endpoint** | `configuration.routes` | `GET /settings` is OWNER-only, so MANAGER/CASHIER screens render the **default** currency symbol, number grouping and date format instead of the store's configured ones. Cosmetic only — every amount is computed server-side and arrives correct — but a store on a non-INR currency shows ₹ to its cashiers. **Planned, additive** `GET /settings/public`. Full spec in [STORE_SETTINGS.md §8](./STORE_SETTINGS.md). |
